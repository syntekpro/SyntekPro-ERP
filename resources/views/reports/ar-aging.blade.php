...
@section('content')
    <section class="space-y-6">
        <div>
            ...
            <h1 class="mt-3 text-4xl font-semibold text-white">Accounts Receivable Aging</h1>
            <p class="mt-3 max-w-2xl text-sm text-stone-300">Outstanding customer credit balances bucketed by days overdue as of {{ $today->toDateString() }}.</p>
        </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-white/10">
                <table class="min-w-full divide-y divide-white/10 text-left text-sm">
                    ...
                    <tbody class="divide-y divide-white/10 bg-stone-950/40 text-stone-200">
                        @forelse ($rows as $row)
                            <tr>
                                <td class="px-4 py-3 text-white">{{ $row['customer_name'] }}</td>
                        ...
                        @empty
                            <tr><td colspan="7" class="px-4 py-10 text-center text-stone-400">No outstanding customer balances.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
